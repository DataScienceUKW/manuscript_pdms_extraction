...
**Background:** Routine clinical data captured in Patient Data Management Systems (PDMS) within intensive care and perioperative settings are an invaluable resource for clinical research. However, the proprietary, fragmented, and transaction-oriented architecture of systems such as Copra severely limits secondary data use and requires extensive Extract, Transform, and Load (ETL) processing.

**Methods:** We developed a modular, Python-based ETL framework utilizing SQLAlchemy to extract high-frequency, multimodal data from our institutional PDMS. The pipeline incorporates domain-specific logic to resolve common documentation inconsistencies, including inference of missing therapy end-times and reconstruction of medication administration rates from proprietary data fields. Extracted data are harmonized into standardized output structures, validated using Pydantic models, and normalized to Central European Time (CET/CEST).

**Results:** The framework successfully transforms raw PDMS data into analysis-ready formats. It provides fine-grained access control, comprehensive audit logging, and salted, irreversible pseudonymization, ensuring full compliance with the European General Data Protection Regulation (DSGVO) and Art. 27 of the Bayerisches Krankenhausgesetz (BayKrG). This structured approach replaces complex ad hoc queries with reproducible, validated, and traceable workflows.

**Conclusion:** The methodology presented here overcomes substantial technical and regulatory barriers to the secondary use of PDMS data. By enabling auditable, harmonized, and governance-compliant access to high-quality clinical data, the framework supports reproducible clinical research and facilitates the development of automation and decision-support systems.
...
To make the extraction framework accessible beyond technically trained users, a lightweight front-end interface was implemented. The front-end interface incorporated fine-grained, user-based access control and audit logging, fulfilling institutional governance requirements as well as the traceability and accountability obligations defined under DSGVO (Art. 5, 32) and Art. 27 BayKrG.
...
With the pipeline described above, we can now run an exemplary data extraction including de-identification and creation of an audit log file (@lst-pipeline_usecase).

:::{#lst-pipeline_usecase}
```python
...
audit.close()
```
Application of the ETL pipeline to extract basic demographics, including audit logging and de-identification
:::

The corresponding audit log (@lst-output_audit) and output file (@lst-output_csv) document all parameters, derived fields, and pseudonymized identifiers, demonstrating transparent, auditable access to high-quality PDMS data suitable for secondary use in research and quality improvement projects.
...
Although de-identification of sensitive parameters via hashing in SQL would be technically feasible, and query-based parameter minimization is obvious, practical implementation and configuration for each individual use case proved highly complex. Furthermore, the use of custom units, varied time formats, and system-specific coding schemes requires extensive data harmonization. Consequently, without dedicated ETL pipelines, these rich datasets remain inaccessible to the majority of clinical researchers. All these hurdles have been described before. [@holmes_why_2021; @lamer_development_2023; @priou_where_2024; @albu_challenges_2025]


### Ethical and Legal Considerations
All data handling complied with the European General Data Protection Regulation (GDPR; Datenschutz-Grundverordnung, DSGVO). The processing of special categories of personal data for scientific research is permitted under Art. 9(2)(j) DSGVO in conjunction with appropriate safeguards, while the general conditions for lawful processing are governed by Art. 6(1) DSGVO. In Bavaria, the specific legal basis enabling hospital-internal research with pseudonymized routine data is Art. 27 Bayerisches Krankenhausgesetz (BayKrG), which permits the secondary use of hospital data for scientific purposes without individual patient consent, provided that suitable technical and organizational measures are implemented. These provisions are complemented by the Bundesdatenschutzgesetz (BDSG) and, in a more general manner, the Bayerisches Datenschutzgesetz (BayDSG), which establish additional requirements for processing personal data in public institutions. At the federal level, the Gesundheitsdatennutzungsgesetz (GDNG) introduces further provisions for the use of health data in research and quality-improvement contexts, thereby reinforcing the legal framework established by the DSGVO and BayKrG.

In line with Art. 32 DSGVO and the security requirements defined in these laws, state-of-the-art technical and organizational measures were applied to ensure confidentiality, integrity, and availability of the data. Patient identifiers were removed at the time of extraction and replaced with salted, irreversible pseudonyms. All data linkage across domains was conducted exclusively within this pseudonymized environment.
