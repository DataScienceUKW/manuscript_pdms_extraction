...
With the pipeline described above, we can now run an exemplary data extraction including de-identification and creation of an audit log file (@pipeline_usecase).
...
The corresponding audit log (@output_audit) and output file (@output_csv) document all parameters, derived fields, and pseudonymized identifiers, demonstrating transparent, auditable access to high-quality PDMS data suitable for secondary use in research and quality improvement projects.
